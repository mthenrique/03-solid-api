...
describe('SignUp', () => {
  ...
  it('should be able to hash a password', async () => {
    ...
    const userWithPassword =
      await usersRepositoryInMemory.findByEmailWithPassword(newUser.email)
  ...
  it('should be able to create a new user', async () => {
    ...
    const user = await usersRepositoryInMemory.findByEmailWithPassword(
      userData.email,
    )
    ...
    expect(
      await compare(userData.password, user?.passwordHash ?? ''),
    ).toBeTruthy()
  })
  ...
  it('should not be able to create a new user with an existing email', async () => {
    ...
    await expect(signUpService.execute(newUser)).rejects.toThrowError(
